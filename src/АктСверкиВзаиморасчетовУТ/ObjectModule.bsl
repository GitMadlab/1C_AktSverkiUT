...
Функция СведенияОВнешнейОбработке() экспорт
    ...
    ПараметрыРегистрации.Вставить("Информация", "Акт сверки взаимных расчетов"); 
...
Процедура Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	//Печатать Факсимиле или Нет...
	//ПараметрыПечати.Вставить("Факсимиле", Истина);
	
	ПараметрыВывода = УправлениеПечатью.ПодготовитьСтруктуруПараметровВывода();
	
	Тема = "Акт сверки взаимных расчетов от "  + МассивОбъектов[0].Организация.НаименованиеСокращенное;
	Текст = "Во вложении акт сверки взаимных расчетов № " + МассивОбъектов[0].Номер + " от " + МассивОбъектов[0].Организация.НаименованиеСокращенное;
	Текст = Текст + ПодписьЭлПисьмо(МассивОбъектов);
	
	// ПрефИмениФайла учавствует в формировании имени файла при сохранении (или отправке по email'лу
	ПрефИмениФайла = МассивОбъектов[0].Контрагент.ИНН + "_(" + СтрЗаменить(МассивОбъектов[0].Контрагент.Наименование, """","") + ")_";
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиВзаимныхРасчетов") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			ПрефИмениФайла + НСтр("ru='АктСверкиВзаиморасчетов'"),
			СформироватьПечатнуюФормуАктСверкиВзаиморасчетов(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
	
	//ФормированиеПечатныхФорм.ЗаполнитьПараметрыОтправки(ПараметрыВывода.ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм);
	
	АдресЭП = ЭлектронноеВзаимодействиеУТ.ПолучитьАдресЭПИзКонтактнойИнформации(МассивОбъектов[0].Контрагент);	
	
	ФормированиеПечатныхФорм.ЗаполнитьПараметрыОтправки(ПараметрыВывода.ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм);
	
	Если НЕ (АдресЭП = Неопределено) Тогда
		ПараметрыВывода.ПараметрыОтправки.Получатель = ЭлектронныйАдресКонтры(ПараметрыВывода.ПараметрыОтправки.Получатель, МассивОбъектов);
	КонецЕсли
...
Функция СформироватьПечатнуюФормуАктСверкиВзаиморасчетов(МассивОбъектов, ОбъектыПечати, ДляОтладки = Неопределено) Экспорт
	...
	Для Каждого ДанныеДокумента Из ДанныеСверки.Строки Цикл
		...
		ВладелецКартинки = Справочники.ФизическиеЛица.НайтиПоНаименованию(ОбластьМакета.Параметры.ФИООтветственногоЛица, Истина);
		КартинкаПуть = ВыводКартинки(ВладелецКартинки);
		ДобавитьФаксимиле(ОбластьМакета, КартинкаПуть, 18.0, 8.0, 15.875, 34.67);
		
		//ВладелецКартинки = Справочники.ФизическиеЛица.НайтиПоНаименованию(ДанныеПечати.ГлавныйБухгалтер, Истина);
		//КартинкаПуть = ВыводКартинки(ВладелецКартинки);
		//ДобавитьФаксимиле(Область, КартинкаПуть, 16.0, 40.0, 15.875, 34.67);
		
		////ВладелецКартинки = Справочники.ФизическиеЛица.НайтиПоНаименованию(ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.Менеджер, ДанныеПечати.Дата), Истина);
		//ВладелецКартинки = ДанныеПечати.Менеджер.Ссылка;
		//КартинкаПуть = ВыводКартинки(ВладелецКартинки);
		//ДобавитьФаксимиле(Область, КартинкаПуть, 27.0, 80.0, 15.875, 34.67);
		
		ВладелецКартинки = ДанныеДокумента.Организация;
		КартинкаПуть = ВыводКартинки(ВладелецКартинки);
		ДобавитьФаксимиле(ОбластьМакета, КартинкаПуть, 6.5, 50.0, 40.0, 40.0);
...
Функция ЭлектронныйАдресКонтры(МассивАдресов, МассивОбъектов)
	
	АдресЭП = ЭлектронноеВзаимодействиеУТ.ПолучитьАдресЭПИзКонтактнойИнформации(МассивОбъектов[0].Контрагент);
	
	Для Индекс = 0 по МассивАдресов.ВГраница() Цикл
		Если СтрНачинаетсяС(МассивАдресов[Индекс].Пояснение, "Контрагент") Тогда
			МассивАдресов[Индекс].Адрес = 	СтрЗаменить(АдресЭП, ",", ";");
		КонецЕсли;
	КонецЦикла;
...
Функция ПодписьЭлПисьмо(МассивОбъектовНазначения)
	
	//ПочтаФилиала = ДополнительныеСведенияЗначение(МассивОбъектовНазначения[0].Ссылка.Склад, "email (Склад (складская территория))");
	//
	//Если ПочтаФилиала = Неопределено Тогда
		АдресЭлектроннойПочты = ПолучитьАдресЭлектроннойПочты(МассивОбъектовНазначения[0].Организация);
	//Иначе
	//	АдресЭлектроннойПочты = СокрЛП(ПочтаФилиала);
	//КонецЕсли;
	
	Подпись = Символы.ПС + "---" + Символы.ПС;
	Подпись = Подпись + "С уважением," + Символы.ПС + МассивОбъектовНазначения[0].Организация.НаименованиеСокращенное + Символы.ПС;
	Подпись = Подпись + "e-mail: " + АдресЭлектроннойПочты + Символы.ПС;
	Подпись = Подпись + ТелефонБизнесРегиона(МассивОбъектовНазначения[0].Партнер.БизнесРегион, "Телефон:") + Символы.ПС;
	Подпись = Подпись + ДополнительныеСведенияЗначение(МассивОбъектовНазначения[0].Организация, "Сайт (Организация)");
	
	Возврат Подпись;
	
КонецФункции

Функция ДополнительныеСведенияЗначение(СсылкаНаОбъект, Свойство)
	...
	Запрос.Текст = 
		...
		|	И ДополнительныеСведения.Объект = &СсылкаНаОбъект";
	...
	//ккк = ВыборкаДетальныеЗаписи.Количество();
...
//
Функция ПолучитьАдресЭлектроннойПочты(Объект)

	Если ЗначениеЗаполнено(Объект) Тогда
		
		//Если ТипАдреса = "Email" Тогда
		//	Начало = "Email";
		...
		ИначеЕсли ТипЗнч(Объект) = Тип("СправочникСсылка.Партнеры") Тогда
			Окончание = "Партнера";
			ВладелецКонтактнойИнформации = Объект;
		//ИначеЕсли ТипЗнч(Объект) = Тип("СправочникСсылка.РегистрацииПодразделенийВИФНС") Тогда
		//	Окончание = "ОбособленногоПодразделения";
		//	ВладелецКонтактнойИнформации = Объект;
		КонецЕсли;
		
		ВидАдреса = Справочники.ВидыКонтактнойИнформации["Email" + Окончание].Ссылка;

		ТабЗн = ВладелецКонтактнойИнформации.КонтактнаяИнформация.Выгрузить();
		
		НайденныеСтроки = ТабЗн.НайтиСтроки(Новый Структура("Тип,Вид", Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ВидАдреса));
...
//Телефон из доп. свойчтв Бизнес-региона
//БизнесРегион - Партнер/БизнесРегион.Ссылка
//Текст для телефона - Строка (Например: "Телефон для связи:")
//
Функция ТелефонБизнесРегиона(Бизнесрегион, ТекстДляТелефона)
	Телефон = СокрЛП(Формат(ДополнительныеСведенияЗначение(Бизнесрегион, "Телефон (Бизнес-регионы)"), "ЧГ=0"));
	Если СтрДлина(Телефон) = 11 Тогда
		Телефон = Лев(Телефон, 1) + " (" + Сред(Телефон, 2, 3) + ") " + Сред(Телефон, 5, 3) + "-" + Сред(Телефон, 8);
	ИначеЕсли СтрДлина(Телефон) = 0 Тогда
		ТекстДляТелефона = "";
		Телефон = "";
	КонецЕсли;
	Если ТекстДляТелефона = "" Тогда
		Текст = "";
	Иначе
		Текст = ТекстДляТелефона + " ";
	КонецЕсли;
	
	Возврат Текст + Телефон;
КонецФункции
...
//Факсимиле
//
Процедура ДобавитьФаксимиле(ОбластьФаксимиле, КартинкаПуть, Верх, Лево, Высота, Ширина)
	//Добавляем картинку...
	
	ФаксимилеПодпись = ОбластьФаксимиле.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Картинка);
	ФаксимилеПодпись.Картинка = Новый Картинка(КартинкаПуть);
	//ФаксимилеПодпись.Картинка = Новый Картинка("d:\Docs\СигКузнецов150х87транс.png");
	ФаксимилеПодпись.Верх = Верх;
	ФаксимилеПодпись.Высота = Высота;
	ФаксимилеПодпись.Ширина = Ширина;
	ФаксимилеПодпись.Лево = Лево;
	ФаксимилеПодпись.ГраницаСверху = Ложь;
	ФаксимилеПодпись.ГраницаСлева = Ложь;
	ФаксимилеПодпись.ГраницаСправа = Ложь;
	ФаксимилеПодпись.ГраницаСнизу = Ложь;
	ФаксимилеПодпись.Узор = ТипУзораТабличногоДокумента.БезУзора;
	//ФаксимилеПодпись.Узор.Безузора;
	ФаксимилеПодпись.РазмерКартинки = РазмерКартинки.АвтоРазмер;
	//Конец Добавляем картинку

КонецПроцедуры

Функция ВыводКартинки(ВладелецФайла)
	
	КартинкаВПамяти = Неопределено;
	
	лТекст = "
		|ВЫБРАТЬ
		|	ДвоичныеДанныеФайлов.Файл.Ссылка КАК ФайлСсылка,
		|	ДвоичныеДанныеФайлов.Файл.Наименование КАК ФайлНаименование,
		|	ДвоичныеДанныеФайлов.Файл.ВладелецФайла КАК ФайлВладелецФайла,
		|	ДвоичныеДанныеФайлов.Файл.ПутьКФайлу КАК ФайлПутьКФайлу,
		|	ДвоичныеДанныеФайлов.Файл.Расширение КАК ФайлРасширение,
		|	ДвоичныеДанныеФайлов.Файл.Код КАК ФайлКод,
		|	ДвоичныеДанныеФайлов.Файл.Владелец КАК ФайлВладелец,
		|	ДвоичныеДанныеФайлов.Файл.ПолноеНаименование КАК ФайлПолноеНаименование,
		...
		|	И ДвоичныеДанныеФайлов.Файл.Наименование ПОДОБНО (&ИмяФайлаСодержит)
		|";

	лЗапрос = Новый Запрос(лТекст);

	лЗапрос.УстановитьПараметр("ВладелецФайла", ВладелецФайла);
	лЗапрос.УстановитьПараметр("ИмяФайлаСодержит", "%faximile%");

	лРезультатЗапроса = лЗапрос.Выполнить();
	Если лРезультатЗапроса.Пустой() Тогда
		//Сообщить("Отсутствует изображение для печати факсимиле!");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "У " + СокрЛП(ВладелецФайла) + " отсутствует изображение для печати факсимиле!";
		Сообщение.Сообщить();
		
		Возврат КартинкаВПамяти;
	...
	Пока лВыборка.Следующий() Цикл
		...
		ФайлСсылка		= лВыборка.ФайлСсылка;
		ФайлИмя  		= лВыборка.ФайлНаименование;
		ВладелецФайла 	= лВыборка.ФайлВладелецФайла;
		Хранилище		= лВыборка.ДвоичныеДанныеФайла;
		ФайлРасширение	= лВыборка.ФайлРасширение;
		
	КонецЦикла;
	
	Попытка
		
	    //Путь = КаталогВременныхФайлов()+ ФайлИмя+"."+ФайлРасширение;
	    КартинкаВПамяти = Хранилище.Получить();
		Файл = Справочники.ВерсииФайлов.НайтиПоНаименованию(ФайлИмя).ФайлХранилище.Получить();
		АдресВХ = ПоместитьВоВременноеХранилище(КартинкаВПамяти);
